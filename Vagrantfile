...
Vagrant.configure(VAGRANTFILE_API_VERSION) do |config|
  ...
  if (RUBY_PLATFORM =~ /linux/) or (RUBY_PLATFORM =~ /darwin/) then
    Dir::mkdir(ENV['HOME'] + '/shared') unless Dir::exist?(ENV['HOME'] + '/shared')
    config.vm.synced_folder '~/shared', '/vagrant_data'
  elsif (RUBY_PLATFORM =~ /win/)
    Dir::mkdir('C:\\shared') unless Dir::exist?('C:\\shared')
    config.vm.synced_folder 'C:\\shared', '/vagrant_data'
  end
